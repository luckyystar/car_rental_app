...
server <- function(input, output, session) {
  ...
  customers_df <- reactivePoll(
    poll_interval_ms, session,
    checkFunc = function() {
      r <- dbGetQuery(con, "SELECT MAX(updated_at) AS last FROM Customers")
      as.character(r$last)
    },
    valueFunc = function() {
...
"))
  ...
  observeEvent(input$update_car_btn, {
    ...
    q <- paste0("UPDATE Cars SET brand=", b, ", model=", m, ", year=", input$car_year,
                ", `type`=", t, ", price_per_day=", input$car_price, ", status=", st)
  ...
  observeEvent(input$book_btn, {
    ...
    if (nrow(cust) == 0) {
      ...
      cust_id <- dbGetQuery(con, "SELECT LAST_INSERT_ID() AS id")$id
    } else {
  ...
  observeEvent(input$delete_booking_btn, {
    ...
    tryCatch({
      ...
      # If no more bookings, delete the customer
      if(remaining == 0) {
